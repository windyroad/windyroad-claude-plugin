...
- ccc1b1a: `/wr-retrospective:run-retro` Step 4a gains sub-step 9 "Prior-session evidence drain (P282)" that consumes durable on-disk evidence — `docs/problems/README.md` Verification Queue rows whose `Likely verified?` cell already records `yes — observed: <citations>` from a prior session — that is structurally invisible to current-session tool-call scans (sub-steps 1-8).
