...
- 7d3bb80: P330 Option B: seed `**Release vehicle**: .changeset/<name>.md` paragraph in ticket Fix Strategy BEFORE the `git mv` to `.verifying.md`.
...
- 8bf3c1d: work-problems Step 6.75: verify-iter-claims sub-step (P335)
  ...
  The verifier (`packages/itil/scripts/verify-iter-summary.sh`, PATH shim `wr-itil-verify-iter-summary`) detects the _emit-but-over-claim_ class — distinct from the _stuck-before-emit_ class (P147) — by combining the commit message + notes against each cited ADR's `## Confirmation` section. Behavioural coverage in `packages/itil/scripts/test/verify-iter-summary.bats` (11 cases) plus a real-world reproduction against the P335 session 8 iter 1 witness commit (252702a / ADR-077).
...
- ac009bb: P346 Phase 3 — capture-time hang-off discipline via fresh-context subagent
...
- ac009bb: P346 Phase 3 — wire hang-off-check subagent into capture/manage Step 2
...
- b160eb8: P347 / ADR-079: Evidence-based relevance-close pass — Phase 2 (4 more evidence shapes + Phase 1 false-positive fixes + structured caveat + KEEP-WITH-NOTE).
  ...
  **User direction (verbatim, 2026-05-31)**: _"Amend the ADR and the implementation"_ — pins Phase 2 substance per ADR-074 build-upon guard.
...
- 3bdd1d7: P347 / ADR-079 Phase 2 — SKILL prose synchronisation (Phase C of the Phase 2 ship).
...
- ef99ea2: P346 / ADR-079: Evidence-based relevance-close pass for the problem backlog (Phase 1: file-no-longer-exists).
  ...
  **User direction (verbatim, 2026-05-31)**: _"Ok, I'm happy for a skill executed as part of review problems that closes tickets that are no longer relevant, but not just because they are old"_ — the relevance signal MUST be observable per ADR-026; age is a **gating** condition (don't bother evaluating fresh tickets), never the **closing** condition.
...
- 96da70d: P267: codify `derive-release-vehicle.sh` helper for deterministic K→V release-cycle citation.
...
- 593716d: Inbound-reported problems now rank ahead of internally-discovered problems (ADR-076; P138 amendment).
...
- 7760599: Adopter-safe path resolution in shipped SKILLs (P317 / RFC-009).
