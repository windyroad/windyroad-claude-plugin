...
- **I13 (trace-to-RFC at fix-time — unconditional)** *(added 2026-05-26 per ADR-071 / RFC-006)*: every problem is fixed only via an RFC — **no carve-out, no effort threshold, no exemption** (per ADR-071; the atomic-fix carve-out originally proposed as RFC-005 F2 is disavowed). A problem MUST trace to ≥ 1 RFC in `proposed` / `accepted` / `in-progress` status before the `Open → Known Error` lifecycle transition (gate placement per ADR-072 — `Known Error` is "fix is now real"; no new lifecycle state). **Hard-block** at `/wr-itil:manage-problem <NNN> --to known-error` AND at the `git commit` that lands the staged `docs/problems/open/ → docs/problems/known-error/` transition (sibling shape to I1's hard-block at `capture-rfc`; extends the gate-surface taxonomy to include `manage-problem <NNN> --to known-error`). The AFK orchestrator (`/wr-itil:work-problems` Step 5) **hard-blocks** dispatch of an RFC-less fix and advances to the next-highest-WSJF candidate — no soft-route auto-capture, because RFC scope is direction-setting (per ADR-073). Problem-ticket frontmatter carries `rfcs: [RFC-<NNN>, ...]` (cardinality `0..N`: `0` permitted pre-`Known Error`; `≥ 1` required at the gate). Symmetric to I1's RFC→Problem direction. There is no `--rfc-deferred` override. The structural hook + skill enforcement ship under RFC-005's B2–B10 task chain (held-changeset window per ADR-042). **Enforcement is load-bearing**: a behavioural test (per ADR-052) asserts the gate fires at every effort level (S/M/L/XL) — no effort skips it.
