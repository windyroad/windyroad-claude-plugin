...
**Refined root cause (P258 investigation 2026-05-26, grounded against the [Claude Code plugins reference](https://code.claude.com/docs/en/plugins-reference))**: `skills` / `agents` / `hooks` / `commands` are *recognised, type-checked* component-path fields (expecting `string | array | object` of component paths or inline config), so supplying maturity-record objects there is a wrong-type **hard load error**. By contrast, *unrecognised* top-level keys are warning-only and the plugin still loads — which is precisely why the §Schema relocation below (nesting under a novel top-level `maturity:` key) is durable: `maturity:` is unrecognised, so it can never trip the type-checker. The documented pre-publish gate is `claude plugin validate --strict` (warnings-as-errors); this refines Confirmation #11's speculative `claude plugin install --dry-run` to the documented surface (see P258 and the structural CI-gate ticket P263).
