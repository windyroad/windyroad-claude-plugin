...
- **Either script's worst-case cost grows above 5 seconds wall-clock on a warm-cache developer laptop**: open a performance-tuning iter and re-quantify under ADR-023's Decision Outcome template. *2026-05-17 update*: trigger fired Phase 2c iter-5 (7.12s observed); Phase 2d iter-6 substring-prefilter optimization shipped, re-quantified at 5.34s warm-cache median (still marginally exceeding the 5s threshold by 0.34s / 6.8%). Phase 2e binary-search-to-first-in-window queued as the next slice within P087; trigger remains advisory-firing pending Phase 2e re-measurement.
