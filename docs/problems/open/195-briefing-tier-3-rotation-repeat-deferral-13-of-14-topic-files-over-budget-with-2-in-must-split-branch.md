...
## Progress — 2026-05-25 briefing-curation pass

Real curation pass run (interactive `/wr-retrospective:run-retro` follow-up, user direction "do a real briefing-curation pass"). Tree-wide: **146,047 → 77,369 bytes (47% reduction)**. Tier-3 over-budget files: **16 → 1** (only `plugin-distribution-cache-mechanics.md` at 6,151 / 1.2× remains — all-durable content + 3 fresh 2026-05-25 entries; no longer MUST_SPLIT). Method: 4 parallel curators applied Step 1.5 decay against the closed/verifying ticket-state index — removed entries whose issue is CLOSED (fix shipped), collapsed verbose verifying-stage workarounds to one-liners, kept durable how-it-works content. Zero unsafe deletions (conservative: keep-when-unsure). NOTE: this is a one-time drain — the recurring fix is that the run-retro Step 3 Tier-3 pass must actually RUN each retro rather than defer (the "no automatic cadence ⇒ never happens" principle; the deferral IS this ticket).

## Update 2026-05-26 — over-budget condition cleared by in-retro tightening

During the 2026-05-26 P283-closeout retro, per direct user direction ("tighten the others too, rather than creating a ticket"), ALL over-budget briefing topic files were tightened under the 5120 Tier-3 threshold in-retro (rather than deferred again). `check-briefing-budgets.sh docs/briefing` now exits with EMPTY output (zero files over budget). Method: deleted 2 redundant entries (a stream-timeout-salvage dup superseded by P261's contractual version; plugin.json-reserved-keys + install-cascade dups now fully covered in the parent `plugin-distribution.md`), and compressed verbose entries across afk-subprocess.md, governance-workflow.md, hooks-and-gates.md, plugin-distribution.md, plugin-distribution-cache-mechanics.md, releases-and-ci.md (preserving signal + citations). The repeat-deferral pattern this ticket tracks was broken by acting (P247 discipline applied). Candidate for close-on-evidence at next review — the snapshot condition (13/14 over, 2 in MUST_SPLIT) is no longer true.
