...
## Follow-on instance (2026-05-26, P283 work)

A sibling zsh-portability instance surfaced during P283 work: `packages/itil/lib/migrate-problems-layout.sh` (mirrored at `packages/shared/`) uses bash-only `shopt -s nullglob` but is **sourced** by `manage-problem` / `work-problems` Step 0a. Under a zsh Bash-tool shell it fails with `command not found: shopt` (hit directly by the orchestrator in Step 0a this session + by the P173 iter; recovered via `bash -c`). **Distinct surface from P133's original scope** (which was zsh-portability of shell-snippet *examples in SKILL.md files*) — this is a sourced executable lib, not a doc snippet — so P133's released fix does not cover it. Tracked here as a related zsh-portability recurrence; may warrant its own fix (wrap the `shopt` in a bash-guard or replace with a portable glob) — re-rate / split at next /wr-itil:review-problems if it should be a standalone ticket rather than a P133 follow-on.
