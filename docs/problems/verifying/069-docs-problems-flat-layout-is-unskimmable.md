...
Fixed — the per-state-subdirectory migration landed via `e31bd6a` "migrate to per-state subdirs (RFC-002 T5)"; the repo and this README now use `docs/problems/<state>/<NNN>-<slug>.md`, and adopter auto-migration shipped in `@windyroad/itil` (ADR-031 / RFC-002). Released in `@windyroad/itil@0.35.8`. Verify: new tickets land in the per-state subdirs, the README renders from the subdir globs, and adopter repos auto-migrate on first run. (P285/P286 flat stragglers migrated 2026-05-26.)
