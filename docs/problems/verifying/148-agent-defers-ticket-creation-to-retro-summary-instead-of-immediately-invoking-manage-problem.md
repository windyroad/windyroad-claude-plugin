...
**Recurrence 2026-05-28 — uncovered surface; DO NOT verify-close the class on the run-retro fix alone.** The class recurred at a surface P148's Fix 1+2 does NOT cover. In an **ad-hoc conversational session-wrap report** (not a `/wr-retrospective:run-retro` invocation), the agent noticed a ticket-worthy issue — the Node-20 GitHub-Actions deprecation (now P325) — and **surfaced it as a "worth a maintenance ticket sometime" recommendation instead of immediately capturing it**. User correction: *"Create the problem ticket for the node 20 stuff. I shouldn't have to ask."* P148's fix is scoped to run-retro Step 4b Stage 1 + `check-tickets-deferred-cause.sh` (which only walks `docs/retros/`); it cannot fire on a conversational session-wrap. **So P148's run-retro surface may verify clean while the class is still open at the conversational surface.** At the next `/wr-itil:review-problems`: decide whether to (a) re-open + broaden P148's scope to "any surface, including conversational session-wrap", or (b) spawn a sibling ticket for the conversational surface — same class, now **three surfaces**: run-retro (P148), Tier-3 rotation (P145), AFK-orchestrator "recommended next step" (P187). The principle is recorded in memory [[feedback_dont_defer_at_session_wrap]]; this is its enforcement-coverage gap at the conversational surface (no mechanical gate there, unlike the retro script).

**Recurrence 2026-05-28 (4th surface — action-completion, not observation-capture).** Same parent class ("agent defers completing instead of acting") at a new facet: the agent **held an authorized irreversible action waiting for a redundant per-step confirm**. After the user directed `graduate it` → `push` → `minimise WIP`, the Version Packages release PR `#168` (CLEAN + MERGEABLE, no blocking checks) was left open with the agent framing the merge as "your call" — until the user asked *"Why is #168 waiting?"*. The merge was the completion of an action authorized four ways across the session; holding it for a fresh confirm over-applied the "confirm irreversible outward actions" default to a *durably-authorized* action. Distinct facet from the observation-capture surfaces above: here the work was authorized and the agent withheld completion, rather than surfacing-instead-of-capturing. Lesson: durable multi-turn authorization of a release means complete the chain (merge/publish) without re-confirming each sub-step. Class now spans FOUR surfaces: run-retro (P148), Tier-3 rotation (P145), AFK-orchestrator recommended-next-step (P187), and authorized-action-completion (this). Reinforces the re-open-or-spawn-sibling decision flagged above — the parent class is broader than the run-retro fix's scope.
