...
- [ ] **Phase 2 implementation — next natural slice** (separated out 2026-05-16 iter-5; previously implicit under "Draft ADR ... measurement mechanism" but the ADR landed without shipping scripts). Ship `packages/itil/bin/wr-itil-skill-invocations` + `packages/itil/scripts/skill-invocations.sh` + `packages/itil/bin/wr-itil-plugin-exercise-index` + `packages/itil/scripts/plugin-exercise-index.sh` per ADR-058 §Script contracts. Bats coverage for nine fixtures listed in ADR-058 §Confirmation (NDJSON-shape, opt-out marker, no-network-primitive, path-hashing, inaccessible-directory, git-axis composite, outside-git-repo, schema-version contract, plus per-script files). Estimated M-effort single iter if scoped to one script + its 5 fixtures and deferring the second script to a sibling iter; M+ if shipping both scripts together in one iter. ADR-035 privacy + ADR-049 shim grammar + ADR-052 behavioural confirmation + ADR-023 performance quantification all binding.
