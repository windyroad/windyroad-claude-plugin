...
- **Release marker**: `@windyroad/itil@0.32.1` (Phase 1 hook originally landed in `@windyroad/itil@0.30.4`, release commit `50626e7`, hook commit `9117246`; current cache version `0.32.1` keeps the hook live).
- **Fix summary**: Phase 1 Option A advisory hook `packages/itil/hooks/itil-fictional-defer-detect.sh` scans `docs/retros/*.md` PostToolUse:Write|Edit for defer-rationale phrases (`next retro` / `next session` / `defer pending` / `defer with cause:` / `deferred per`) lacking a SCHEDULED-FUTURE-SURFACE citation (ticket ID, named skill invocation, hook path, CI workflow path, or dated ADR) in a ±5-line context window; allowlists the `deferred per Branch B` SKILL-contracted surface. Advisory-only, ADR-045 honour-system budget.
- **Awaiting user verification** — confirm no analogous fictional-defer recurrence across the next observed retro that exercises Tier 3 budget rotation / Signal-vs-Noise pass / Step 4b Stage 1 Tickets Deferred surfaces.
- **Exercise evidence (observed this session)**:
  - `docs/retros/2026-05-17-session-4-iter-5-p233.md` line 46 — Tier 3 budget pass exercised; 14 OVER files all deferred per Branch B `leave-as-is` (legitimate scheduled-future-surface: next `check-briefing-budgets.sh` invocation).
  - `docs/retros/2026-05-17-session-4-iter-6-p162-phase-2b.md` line 21 — Tier 3 budget pass exercised; deferred per Branch B `leave-as-is` with explicit P235 citation as scheduled-future-surface for the broader backlog.
  - `docs/retros/2026-05-17-session-4-wrap.md` line 94 — "Next retro should rotate proactively rather than waiting for the Branch A force" — recommendation tied to the Branch A SKILL contract (the `check-briefing-budgets.sh`-driven force trigger IS the scheduled-future-surface); not a fictional defer, since the cited mechanism is concrete and addressable.
  - 0 fictional-defer recurrences observed across iters 5 / 6 / 7 / 8 / wrap retros post-Phase-1-ship.

## Change Log

- **2026-05-17 (iter 5 — Known Error → Verification Pending transition)** — `/wr-itil:work-problems` AFK iter 5 advanced Status Known Error → Verification Pending after empirical verification criterion met: Phase 1 hook `itil-fictional-defer-detect.sh` (active in cache at `@windyroad/itil@0.32.1`) observed across 5 subsequent retros post-iter-4 transition (iters 5 / 6 / 7 / 8 / session-4-wrap), with Tier 3 budget rotation surface exercised in iters 5 + 6 and session-4-wrap. Zero analogous fictional-defer recurrences across that window; all defer entries in those retros cite legitimate SCHEDULED-FUTURE-SURFACE (Branch B `check-briefing-budgets.sh` invocation, explicit P235 ticket citation, or Branch A SKILL-contracted force trigger). Pre-flight checks per `/wr-itil:transition-problem` Step 4 for `verifying` destination: (1) fix implemented [yes — Phase 1 hook shipped iter 3 commit `9117246`, released iter 4 `@windyroad/itil@0.30.4` commit `50626e7`, current cache `0.32.1`], (2) release marker available [yes — `@windyroad/itil@0.32.1`]. File renamed `docs/problems/known-error/234-...md` → `docs/problems/verifying/234-...md` (per-state subdir layout per ADR-031 / RFC-002 T5a); Status field updated Known Error → Verification Pending; `## Fix Released` section appended naming release marker + fix summary + awaiting-verification line + exercise evidence (4 retro citations); this Change Log entry appended. README.md regenerated — row moves from WSJF Rankings (Known Error) → Verification Queue, `Likely verified?` cell populated with `yes — observed:` evidence-first wording per P186 since the verification criterion has empirical evidence. Last-reviewed line rotated per P134 (prior iter-4 Open → Known Error fragment displaced to `docs/problems/README-history.md`). Phase 2 (Option B `check-defer-rationales.sh` cross-session trend detector) + Phase 3 (Option C SKILL.md prose hardening) remain CONDITIONAL backlog per the P179 carve-out — neither has graduated; SCHEDULED-FUTURE-SURFACE for those phases = orchestrator WSJF queue's re-selection of P234 at recurrence trigger. Architect/JTBD reviews skipped per `/wr-itil:transition-problem` SKILL contract (metadata-only transition; `docs/problems/` excluded from architect/JTBD gates); risk-scorer commit gate satisfied per Step 8. Partial-pathspec staging used per orchestrator dirty-for-known-reason constraint (P234 + README + history paths only; pre-existing 9 staged renames + `.claude/settings.json` mod preserved untouched).
